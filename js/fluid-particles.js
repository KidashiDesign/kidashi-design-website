...
particleDensity:     options.particleDensity     ?? 50,
    ...
    this._cw         = 0 // Cache canvas CSS width (avoid layout reads in the animation loop)
    this._ch         = 0 // Cache canvas CSS height
  ...
  _resize() {
    // Cap the device pixel ratio: clearing/filling the canvas scales with pr²,
    ...
    _animate() {
    const ctx = this.ctx
    // Use the cached CSS-pixel size from the last resize instead of reading
    // offsetWidth/offsetHeight here, which would force a synchronous layout
    ...
    // Stop animating while the tab is hidden — nothing is painted anyway,
    // so this only saves CPU/battery and has no visual effect.
    ...
    this._handlers = { onResize, onScroll, onMouseMove, onClick, onTouchMove, onTouchStart, onTouchEnd, onVisibilityChange }
    this._resize()
    this._animate()
  }
    destroy() {
