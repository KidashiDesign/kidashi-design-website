...
## 10. Priority 4 — implemented, with two items flagged rather than built (2026-08-31)

- **FAQ expanded to the master brief's full minimum coverage.** The shared FAQ on `/services/` (already the studio-level FAQ home, reused from `/services/` per the "check for an existing system first" instruction) had 4 of the 9 required topics — added the missing 5: international work, how to start a project, what the process looks like, project timelines, and rebrand capability. Every new answer is drawn from facts already published elsewhere on the site (e.g. the process answer restates the existing 5-stage "My Process" section verbatim in substance; the international-work answer restates the "partners with clients internationally" line already on `/about/` and in the homepage meta description) — nothing new was asserted. The timeline answer deliberately stays general ("timelines vary by project scope, confirmed during the initial briefing") rather than inventing a number of weeks, since no committed turnaround time exists anywhere in the repo. Matching `FAQPage` JSON-LD updated in lockstep (9 visible items, 9 schema questions, verified 1:1). In the same edit, corrected a pre-existing schema slip where this FAQ's `@id` pointed at the homepage fragment (`#/faq`) instead of its own page.
- **About page:** already covered who Nicole is, her background, specialties, and design approach in detail (from earlier sessions and Priority 1). The one genuine gap against the brief's checklist was "typical clients/projects" — added one sentence naming the industries her portfolio actually covers (hospitality, travel, private aviation — all real, existing case studies) with a descriptive link to the full portfolio.
- **Process page: not created, decision flagged rather than made unilaterally.** The business's process already exists in crawlable form (`/services/` "My Process" — 5 stages: Discovery & Briefing, Design Concept, Content & Copy, Build & Launch, Support & Growth) plus a shorter 4-step version tailored to each individual service page (added in Priority 2). A dedicated standalone `/process/` page would mean a new route, a new nav entry duplicated across 24 pages, a new sitemap entry, and a new canonical — a materially bigger structural change than anything else in this project so far, for content that's already accessible and crawlable today. Given "modify the smallest reasonable number of files" and "don't rebuild from scratch," I did not build this — flagging it for your decision instead of assuming.
- **Resources section: not created — the brief marks this explicitly optional, and none exists today.** Same reasoning: a genuinely useful resources section is a real content investment (new pages, new routes, new nav/sitemap entries), not a copy tweak, so I'm not building it speculatively. Happy to scope it if you want it.
