...
4. **Alle 9 Kachel-Vorschauen auf WebP-Loop umgestellt.** `xp-days`, `rohyma-jet`, `selvoma`, `galerie-kronsbein`, `tm-studio`, `studio995`, `art-gerecht-modular` zeigen im Grid jetzt ein per Playwright aufgenommenes, animiertes WebP (640×360, 8fps, 6–8s Loop, Pillow `quality=50`) statt eines vollen React-iframes — `images/portfolio/{projekt}/tile-preview.webp`, 85 KB–620 KB statt 0,8–3,8 MB. Volle Animation bleibt unverändert auf der jeweiligen Detailseite. `seestern` und `hideout-georgia` bleiben unverändert als iframe (bereits nativ leichtgewichtig, ~18–23 KB, kein Umstellungsbedarf). Galerie Kronsbein nutzt dieselbe Datei für Desktop-Crop **und** Phone-Mockup (kein JS mehr nötig, um eins von beiden auszuwählen). Bei `art-gerecht-modular` (6-Szenen-Sequenz, ~20s Vollzyklus) wurde bewusst nur die Eröffnung der Sequenz (Intro-Szene) als Loop eingefangen, nicht der komplette Zyklus.
   - Capture-Pipeline (nicht mehr im Repo, bei Bedarf neu schreiben): Playwright `page.screenshot()` in fester Frequenz → PNG-Sequenz → `Image.save(..., save_all=True, duration=125, loop=0, quality=50, method=6)` via Pillow.
   - Kein `ffmpeg` in dieser Sandbox verfügbar; `pip install Pillow` funktioniert aber (PyPI ist nicht wie unpkg.com blockiert) — WebP statt MP4 macht daher auch ffmpeg-frei reproduzierbar.
