...
4. **Kachel-Vorschau auf WebP-Loop umgestellt** (die 4 Kandidaten mit bestehendem Tile-Modus): `xp-days`, `rohyma-jet`, `selvoma`, `galerie-kronsbein` zeigen im Grid jetzt ein per Playwright aufgenommenes, animiertes WebP (640×360, 8fps, 6s Loop, Pillow `quality=50`) statt eines vollen React-iframes — `images/portfolio/{projekt}/tile-preview.webp`, 85–420 KB statt 0,8–3,8 MB. Volle Animation bleibt unverändert auf der jeweiligen Detailseite. Galerie Kronsbein nutzt dieselbe Datei für Desktop-Crop **und** Phone-Mockup (kein JS mehr nötig, um eins von beiden auszuwählen).
   - **Noch offen:** `tm-studio`, `studio995` (kein bestehender Tile-Modus) und `art-gerecht-modular` (6 Szenen, kein Tile-Modus, aufwendigster Fall) sind noch nicht umgestellt — gleiche Capture-Pipeline anwendbar (`/tmp/.../scratchpad/capture_frames2.js` + Pillow-Snippet in dieser Session, nicht mehr im Repo).
